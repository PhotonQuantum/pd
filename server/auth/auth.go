...
func newUserManager(kv kv.Base) *userManager {
	return &userManager{kv: kv, mu: new(sync.RWMutex), users: make(map[string]*User)}
}
...
func (m *userManager) SetRoles(name string, roles map[string]struct{}) error {
	user, err := m.GetUser(name)
	if err != nil {
		return err
	}
...
func newRoleManager(kv kv.Base) *roleManager {
	return &roleManager{kv: kv, mu: new(sync.RWMutex), roles: make(map[string]*Role)}
}
